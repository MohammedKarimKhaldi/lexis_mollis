...
def _build_type_profile(
    ...
    for doc_id in doc_ids:
        ...
        doc_quality = [
            chunk.get("quality_score") for chunk in doc_chunks if isinstance(chunk.get("quality_score"), (int, float))
        ]
        if doc_quality:
        ...
        pages.append(page_counts.get(doc_id) or max((chunk.get("page_number") or 0) for chunk in doc_chunks) or 1)
    ...
    for key, label, _pattern in compiled:
        ...
        if key in COUNT_MARKERS:
            entry["avg_occurrences_per_document"] = round(marker_counts.get(key, 0) / n_docs, 2) if n_docs else 0.0
...
def build_doc_type_profiles(
    ...
    types_profile = {
        doc_type: _build_type_profile(
            doc_type, doc_ids, by_doc, page_counts, compiled, mapping, sample_chars, max_samples, min_documents
        )
